 ...
 * a std::vector<ck_tile::GroupedGemmHostArgs<>> with rcr default strides
 * (stride_A=K, stride_B=K, stride_C=N, k_batch=1) and launch. After the launch the
 * per-group C buffers are copied back to the caller's host buffers.
 *
                                ...
                                float* time_ms)
{
    ...
    for(int g = 0; g < group_count; ++g)
    {
        ...
        // C row-major (stride=N). k_batch=1 for numeric parity.
        descs.emplace_back(static_cast<const void*>(A_dev[g]),
                           ...
                           /*stride_E=*/static_cast<ck_tile::index_t>(N));
    }
