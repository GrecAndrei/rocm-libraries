...
Input JSON format:
    Single: {"so_path": "...", "problem": {"groups": [[M,N,K], ...]}, "kernel_name": "..."}
...
def _run_one(idx, so_path, prob_dict, kernel_name):
    ...
    try:
        ...
        for (M, N, K) in problem.groups:
            A_list.append((np.random.randn(M, K) * 0.1).astype(np.float16))
            B_list.append((np.random.randn(K, N) * 0.1).astype(np.float16))

        # CRITICAL: load the library ONLY inside this subprocess.
        runner = GpuGroupedGemmRunner(lib_path=so_path)
